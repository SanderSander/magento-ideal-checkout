<?php
	...
	class Mage_Idealcheckoutcartebleue_Model_Idealcheckoutcartebleue extends Mage_Payment_Model_Method_Abstract
	{
		...
		// This url is called after order confirmation; It triggers IdealcheckoutcartebleueController->redirectAction()
		public function getOrderPlaceRedirectUrl() 
		{
			$sUrl = Mage::getUrl('idealcheckoutcartebleue/idealcheckoutcartebleue/redirect', array('_secure' => true));
			return $this->fixUrl($sUrl, false);
		}
	


		// Setup idealcheckoutcartebleue transaction record in database - called in: IdealcheckoutcartebleueController->redirectAction()
		public function setupPayment()
		{
			...
			{
				// Mage::throwException(Mage::helper('idealcheckoutcartebleue')->__('The total amount of order #' . $iOrderId . ' is ' . $this->_order->getGrandTotal() . ', but should be at least 1.00.'));
			}
			...
			$sReturnUrl = Mage::getUrl('idealcheckoutcartebleue/idealcheckoutcartebleue/return', array('_secure' => true, 'order_id' => $sOrderId, 'order_code' => $sOrderCode));
...
`transaction_log` = NULL;";
		...
		// Validate payment after user returns from Carte Bleue - called in: IdealcheckoutcartebleueController->returnAction()
