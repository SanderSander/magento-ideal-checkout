<?php
	...
	class Mage_Idealcheckoutpostepay_Model_Idealcheckoutpostepay extends Mage_Payment_Model_Method_Abstract
	{
		...
		// This url is called after order confirmation; It triggers IdealcheckoutpostepayController->redirectAction()
		public function getOrderPlaceRedirectUrl() 
		{
			$sUrl = Mage::getUrl('idealcheckoutpostepay/idealcheckoutpostepay/redirect', array('_secure' => true));
			return $this->fixUrl($sUrl, false);
		}
	


		// Setup idealcheckoutpostepay transaction record in database - called in: IdealcheckoutpostepayController->redirectAction()
		public function setupPayment()
		{
			...
			{
				// Mage::throwException(Mage::helper('idealcheckoutpostepay')->__('The total amount of order #' . $iOrderId . ' is ' . $this->_order->getGrandTotal() . ', but should be at least 1.00.'));
			}
			...
			$sReturnUrl = Mage::getUrl('idealcheckoutpostepay/idealcheckoutpostepay/return', array('_secure' => true, 'order_id' => $sOrderId, 'order_code' => $sOrderCode));
...
`transaction_log` = NULL;";
		...
		// Validate payment after user returns from PostePay - called in: IdealcheckoutpostepayController->returnAction()
