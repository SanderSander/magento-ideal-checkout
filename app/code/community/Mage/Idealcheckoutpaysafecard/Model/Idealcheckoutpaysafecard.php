<?php
	...
	class Mage_Idealcheckoutpaysafecard_Model_Idealcheckoutpaysafecard extends Mage_Payment_Model_Method_Abstract
	{
		...
		// This url is called after order confirmation; It triggers IdealcheckoutpaysafecardController->redirectAction()
		public function getOrderPlaceRedirectUrl() 
		{
			$sUrl = Mage::getUrl('idealcheckoutpaysafecard/idealcheckoutpaysafecard/redirect', array('_secure' => true));
			return $this->fixUrl($sUrl, false);
		}
	


		// Setup idealcheckoutpaysafecard transaction record in database - called in: IdealcheckoutpaysafecardController->redirectAction()
		public function setupPayment()
		{
			...
			{
				// Mage::throwException(Mage::helper('idealcheckoutpaysafecard')->__('The total amount of order #' . $iOrderId . ' is ' . $this->_order->getGrandTotal() . ', but should be at least 1.00.'));
			}
			...
			$sReturnUrl = Mage::getUrl('idealcheckoutpaysafecard/idealcheckoutpaysafecard/return', array('_secure' => true, 'order_id' => $sOrderId, 'order_code' => $sOrderCode));
...
`transaction_log` = NULL;";
		...
		// Validate payment after user returns from PaySafeCard - called in: IdealcheckoutpaysafecardController->returnAction()
