<?php
	...
	class Mage_Idealcheckoutvisa_Model_Idealcheckoutvisa extends Mage_Payment_Model_Method_Abstract
	{
		...
		// This url is called after order confirmation; It triggers IdealcheckoutvisaController->redirectAction()
		public function getOrderPlaceRedirectUrl() 
		{
			$sUrl = Mage::getUrl('idealcheckoutvisa/idealcheckoutvisa/redirect', array('_secure' => true));
			return $this->fixUrl($sUrl, false);
		}
	


		// Setup idealcheckoutvisa transaction record in database - called in: IdealcheckoutvisaController->redirectAction()
		public function setupPayment()
		{
			...
			{
				// Mage::throwException(Mage::helper('idealcheckoutvisa')->__('The total amount of order #' . $iOrderId . ' is ' . $this->_order->getGrandTotal() . ', but should be at least 1.00.'));
			}
			...
			$sReturnUrl = Mage::getUrl('idealcheckoutvisa/idealcheckoutvisa/return', array('_secure' => true, 'order_id' => $sOrderId, 'order_code' => $sOrderCode));
...
`transaction_log` = NULL;";
		...
		// Validate payment after user returns from Visa - called in: IdealcheckoutvisaController->returnAction()
