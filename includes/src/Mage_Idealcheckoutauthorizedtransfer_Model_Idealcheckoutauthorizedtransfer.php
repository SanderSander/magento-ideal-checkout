<?php
	...
	class Mage_Idealcheckoutauthorizedtransfer_Model_Idealcheckoutauthorizedtransfer extends Mage_Payment_Model_Method_Abstract
	{
		...
		// This url is called after order confirmation; It triggers IdealcheckoutauthorizedtransferController->redirectAction()
		public function getOrderPlaceRedirectUrl() 
		{
			$sUrl = Mage::getUrl('idealcheckoutauthorizedtransfer/idealcheckoutauthorizedtransfer/redirect', array('_secure' => true));
			return $this->fixUrl($sUrl, false);
		}
	


		// Setup idealcheckoutauthorizedtransfer transaction record in database - called in: IdealcheckoutauthorizedtransferController->redirectAction()
		public function setupPayment()
		{
			...
			{
				// Mage::throwException(Mage::helper('idealcheckoutauthorizedtransfer')->__('The total amount of order #' . $iOrderId . ' is ' . $this->_order->getGrandTotal() . ', but should be at least 1.00.'));
			}
			...
			$sReturnUrl = Mage::getUrl('idealcheckoutauthorizedtransfer/idealcheckoutauthorizedtransfer/return', array('_secure' => true, 'order_id' => $sOrderId, 'order_code' => $sOrderCode));
...
`transaction_log` = NULL;";
			...
			// Return Eenmalige machtiging URL
		...
		// Validate payment after user returns from Eenmalige machtiging - called in: IdealcheckoutauthorizedtransferController->returnAction()
