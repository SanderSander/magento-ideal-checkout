<?php
	...
	class Mage_Idealcheckoutmastercard_Model_Idealcheckoutmastercard extends Mage_Payment_Model_Method_Abstract
	{
		...
		// This url is called after order confirmation; It triggers IdealcheckoutmastercardController->redirectAction()
		public function getOrderPlaceRedirectUrl() 
		{
			$sUrl = Mage::getUrl('idealcheckoutmastercard/idealcheckoutmastercard/redirect', array('_secure' => true));
			return $this->fixUrl($sUrl, false);
		}
	


		// Setup idealcheckoutmastercard transaction record in database - called in: IdealcheckoutmastercardController->redirectAction()
		public function setupPayment()
		{
			...
			{
				// Mage::throwException(Mage::helper('idealcheckoutmastercard')->__('The total amount of order #' . $iOrderId . ' is ' . $this->_order->getGrandTotal() . ', but should be at least 1.00.'));
			}
			...
			$sReturnUrl = Mage::getUrl('idealcheckoutmastercard/idealcheckoutmastercard/return', array('_secure' => true, 'order_id' => $sOrderId, 'order_code' => $sOrderCode));
...
`transaction_log` = NULL;";
		...
		// Validate payment after user returns from Mastercard - called in: IdealcheckoutmastercardController->returnAction()
